...
pub static LOG_PATH: LazyLock<String> = LazyLock::new(|| {
    std::env::var("LOG_PATH").unwrap_or_else(|_| "/config/access.log".to_string())
});

pub static DATA_DIR: LazyLock<String> =
    LazyLock::new(|| std::env::var("DATA_DIR").unwrap_or_else(|_| "./data".to_string()));

pub static GEOIP_DB: LazyLock<Option<String>> = LazyLock::new(|| std::env::var("GEOIP_DB").ok());
...
pub static OLLAMA_MODEL: LazyLock<String> =
    LazyLock::new(|| std::env::var("OLLAMA_MODEL").unwrap_or_else(|_| "llama3.2".to_string()));
...
pub static OIDC_CLIENT_ID: LazyLock<Option<String>> = LazyLock::new(|| {
...
pub static OIDC_SCOPES: LazyLock<String> = LazyLock::new(|| {
...
pub static OIDC_ADMIN_CLAIM: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("OIDC_ADMIN_CLAIM")
        .ok()
        .filter(|s| !s.is_empty())
});

/// Value within OIDC_ADMIN_CLAIM that grants admin (e.g. "admin").
pub static OIDC_ADMIN_VALUE: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("OIDC_ADMIN_VALUE")
        .ok()
        .filter(|s| !s.is_empty())
});
...
pub static OIDC_PROVIDER_LOGO_URL: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("OIDC_PROVIDER_LOGO_URL")
        .ok()
        .filter(|s| !s.is_empty())
});
